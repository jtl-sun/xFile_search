...
func TestListRowFromSearchArrow(t *testing.T) {
	tests := []struct { name string; dir, current, count, want int }{
	...
	for _, tc := range tests { t.Run(tc.name, func(t *testing.T) { if got := listRowFromSearchArrow(tc.dir, tc.current, tc.count); got != tc.want { t.Fatalf("got %d, want %d", got, tc.want) } }) }
}
