...
일반 Windows 사용자는 GitHub **Releases**에서 **`xFile_search_Setup_v0.1.29_x64.exe`** 하나만 다운로드해 실행하면 됩니다. 관리자 권한 없이 사용자 계정에 설치되며, 업데이트할 때 기존 Index와 검색 기록을 보존합니다.

무설치 사용자는 **`xFile_search_Portable_v0.1.29_x64.zip`**을 받아 압축 해제 후 `xFile_search.exe`를 실행하면 됩니다. 자세한 내용은 [INSTALL_KO.md](INSTALL_KO.md)를 참고하세요.
...
- 인덱싱 중 **굵은 초록색 `INDEXING... xx%` 표시**, 창 제목의 퍼센트, `Indexing...` 버튼, 하단 Marquee 진행 표시, 현재 스캔 위치, 누적 파일 수, skipped 수를 표시
